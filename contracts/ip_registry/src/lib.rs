...
impl IpRegistry {
    ...
    pub fn revoke_ip(env: Env, ip_id: u64) {
        ...
        // Issue #815: Prune the revoked commitment hash from CommitmentHashes.
        // The rollback-protection checksum only needs active (non-revoked)
        // commitment hashes: a revoked hash can never be re-used because
        // CommitmentOwner(hash) still exists in storage as a permanent duplicate
        // guard.  Removing the hash from CommitmentHashes keeps the vector
        // bounded and prevents unbounded ledger growth.
        Self::prune_commitment_hash(&env, &revoked_hash);
    }

    /// Issue #815: Remove a single commitment hash from the CommitmentHashes
    /// tracking vector and recompute the IpCommitmentChecksum.
    ///
    /// Safety: `CommitmentOwner(hash)` remains in storage as the permanent
    /// collision/duplicate guard, so pruning from this list does NOT break
    /// rollback protection for still-active IPs.
    fn prune_commitment_hash(env: &Env, hash_to_remove: &BytesN<32>) {
        ...
        // Rebuild the vector without the revoked hash.
        ...
        if pruned.len() != hashes.len() {
            env.storage()
                .persistent()
                .set(&DataKey::CommitmentHashes, &pruned);
            env.storage().persistent().extend_ttl(
            ...
            // Recompute checksum over the remaining active hashes.
            let mut all_bytes = Bytes::new(env);
            for h in pruned.iter() {
                all_bytes.append(&h.into());
            }
            let checksum: BytesN<32> = if all_bytes.is_empty() {
                BytesN::from_array(env, &[0u8; 32])
            } else {
                env.crypto().sha256(&all_bytes).into()
            };
            env.storage()
                .persistent()
